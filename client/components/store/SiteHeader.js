...
export function SiteHeader() {
  ...
  return (
    <>
      ...
      <div className="bg-ink text-primary-foreground">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2 text-[12px]">
          <p className="hidden sm:block opacity-80">
            🚚 Free delivery on orders over $50 · Returns within 14 days
          </p>
          <p className="sm:hidden opacity-80">🚚 Free delivery over $50</p>
          <div className="flex items-center gap-3">
            <Link
              href="/contact"
              className="hidden md:inline opacity-80 hover:opacity-100"
            >
              Sell on Merkato
            </Link>
            <span className="opacity-60 hidden md:inline">|</span>
            <Link href="/help-center" className="opacity-80 hover:opacity-100">
              Help
            </Link>
          </div>
        </div>
      ...
      >
        <div
          className={`mx-auto max-w-7xl px-4 transition-all ${scrolled ? "py-2.5" : "py-4"}`}
        >
                ...
                {searchOpen && (
                  <div className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-2xl border border-border bg-card shadow-(--shadow-elegant)">
                    {suggestions.length > 0 ? (
                      <div>
                        <p className="px-4 pt-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                          Products
                        </p>
                        {suggestions.map((p) => (
                          <Link
          ...
          <nav className="no-scrollbar mt-3 hidden items-center gap-6 overflow-x-auto text-sm font-medium text-foreground/80 md:flex">
            <Link
              href="/categories"
              className="whitespace-nowrap py-1.5 font-semibold transition hover:text-primary"
            >
              All Categories
            </Link>
            {CATEGORY_LIST.map((c) => (
              <Link
                ...
                className="whitespace-nowrap py-1.5 transition hover:text-primary"
              >
                {c.name}
              </Link>
            ))}
            ...
            <Link
              href="/deals"
              className="whitespace-nowrap py-1.5 font-semibold text-accent"
            >
              Flash Deals 🔥
            </Link>
            <Link
          ...
          <div className="border-t border-border bg-card md:hidden">
            <nav className="mx-auto max-w-7xl px-4 py-4">
              <div className="space-y-1 text-sm font-medium">
                ...
                <Link
                  href="/deals"
                  className="block rounded-lg px-3 py-2 text-accent hover:bg-secondary"
                >
                  Flash Deals 🔥
                </Link>
                <Link
